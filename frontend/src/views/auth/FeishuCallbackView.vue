        ...
        >
          ...
          <template v-if="needsInvitation">
            <p class="text-sm text-gray-700 dark:text-gray-300">
              {{ t('auth.feishu.invitationRequired') }}
            </p>
            <div>
              <input
            ...
            >
              {{ isSubmitting ? t('auth.feishu.completing') : t('auth.feishu.completeRegistration') }}
            </button>
          </template>
...
type FeishuPendingActionResponse = PendingOAuthExchangeResponse & {
  step?: string
  intent?: string
  email?: string
...
function extractPendingAccountEmail(completion: FeishuPendingActionResponse): string {
  return (
...
function resolvePendingAccountAction(
  completion: FeishuPendingActionResponse
...
function applyPendingAccountAction(completion: FeishuPendingActionResponse) {
...
function applyTotpChallenge(completion: FeishuPendingActionResponse): boolean {
...
async function finalizePendingAccountResponse(completion: FeishuPendingActionResponse) {
  applyAdoptionSuggestionState(completion)
  const redirect = sanitizeRedirectPath(completion.redirect || redirectTo.value)

  // 飞书登录始终携带企业邮箱，不存在 email_completion 补邮箱环节。
...
async function handleSubmitInvitation() {
  ...
  try {
    ...
    const { data: completion } = await apiClient.post<FeishuPendingActionResponse>(
      '/auth/oauth/feishu/complete-registration',
      {
    ...
    invitationError.value =
      err.response?.data?.message || err.message || t('auth.feishu.completeRegistrationFailed')
  } finally {
...
async function handleCreateAccount(payload: PendingOAuthCreateAccountPayload) {
  ...
  isSubmitting.value = true
  try {
    const { data } = await apiClient.post<FeishuPendingActionResponse>('/auth/oauth/pending/create-account', {
      email: payload.email,
...
async function handleBindLogin() {
  ...
  isSubmitting.value = true
  try {
    const { data } = await apiClient.post<FeishuPendingActionResponse>('/auth/oauth/pending/bind-login', {
      email,
      password,
      ...serializeAdoptionDecision(currentAdoptionDecision())
    })
...
onMounted(async () => {
  ...
  try {
    ...
    if (error) {
      const i18nKey = `auth.feishu.error.${error}`
    ...
    if (applyTotpChallenge(completion as FeishuPendingActionResponse)) {
      persistPendingAuthSession(completionRedirect)
      return
    }

    applyPendingAccountAction(completion as FeishuPendingActionResponse)
    if (pendingAccountAction.value !== 'none') {
      isProcessing.value = false
      persistPendingAuthSession(completionRedirect)
      return
